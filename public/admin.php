...
if (! is_user_logged_in()) {
    redirect_with_message('/index.php', 'Please log in to access this page.');
}
...
if (is_post_request()) {
    ...
    if (empty($errors)) {
        ...
        $status = (isset($_SESSION['is_admin']) && $_SESSION['is_admin']) ? 'approved' : 'pending';
        $conn = Database::getConnection();
        $stmt = $conn->prepare('
        ...
        header('Content-Type: application/json');
        echo json_encode(['success' => true, 'message' => 'Café added successfully!']);
        exit;
    } else {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'errors' => $errors]);
...
$districts = Database::getConnection()->query('SELECT id, name FROM districts')->fetchAll(PDO::FETCH_ASSOC);

require_once __DIR__.'/../app/views/layouts/header.php';
require_once __DIR__.'/../app/views/admin/adminDashboard.php';
